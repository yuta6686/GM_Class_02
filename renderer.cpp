...
HRESULT Renderer::CreateStructuredBuffer(UINT uElementSize, UINT uCount, void* pInitData, ID3D11Buffer** ppBufOut)
{
	...
	if (pInitData) {
		...
		return m_Device->CreateBuffer(&desc, &InitData, ppBufOut);
	}
	else {
		return m_Device->CreateBuffer(&desc, nullptr, ppBufOut);
	}
}
...
HRESULT Renderer::CreateBufferSRV(ID3D11Buffer* pBuffer, ID3D11ShaderResourceView** ppSRVOut)
{
	...
	return m_Device->CreateShaderResourceView(pBuffer, &desc, ppSRVOut);
}
